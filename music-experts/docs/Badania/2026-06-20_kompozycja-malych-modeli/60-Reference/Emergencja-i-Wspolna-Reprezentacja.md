...
**Sweep skali UTWARDZONY (0,05 / 0,2 / 0,8 / 1,8M, same-task inny seed, 3 seedy/skalę → mean±std):** CKA **0,952 → 0,964 → 0,973 → 0,971** (przy suficie od 0,05M — efekt sufitu), ale **mutual-kNN rośnie monotonicznie i PONAD szum: 0,713±0,018 → 0,793 → 0,829 → 0,842±0,002** — czysty kierunek PRH widoczny **tylko czulszą metryką**. Metryka ma znaczenie (zmierzone). Konwergencja **wczesna** (CKA ~0,95 już na 0,05M). ⚠️ wciąż 1 domena (jigi) + zakres 0,05–1,8M; cross-domena i 3M+ = dalej. Figura: `paper/paper.tex` (`fig:cka`).
