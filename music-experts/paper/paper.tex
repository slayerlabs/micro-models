...
\paragraph{Decydujący tani pomiar następny.} Zanim budować bogatszy mechanizm, warto zmierzyć, czy niezależnie trenowane mikro-modele w ogóle dzielą geometrię reprezentacji --- metryką CKA \cite{cka} / mutual-kNN, i to jako \emph{funkcję skali}. Hipoteza platońska \cite{platonic} przewiduje konwergencję rosnącą z rozmiarem; jej zachowanie na skali $\sim$$10^6$ parametrów jest niezbadane. Brak konwergencji tłumaczyłby nasz remis wprost (różne geometrie $\Rightarrow$ stitch $\approx$ ensemble); jej obecność otwierałaby darmową interoperacyjność przez reprezentacje względne \cite{relreps}. \emph{Uwaga metodyczna do dalszego routingu:} domeny rozłączne po metrum (6/8 vs 4/4) pozwalają routerowi „oszukać'' po jednym tokenie nagłówka --- testy routingu wymagają domen w \emph{tym samym} metrum.
